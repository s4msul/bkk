    ...
    <script type="text/javascript">
        $(function() {
            ...
            $('#bt_tambah').click(function() {
                $('#form_add').modal('show');
                reset_form();
                $('#form_add .title h4').html('Form Tambah lamaran');
            });
        ...
        function get_list_lamaran(p) {
            $('#form-pencarian').modal('hide');
            var id = '';
            $.ajax({
                type : 'GET',
                url: '<?= base_url("api/areafiftyone/lamarans") ?>/page/'+p+'/id/'+id,
                data: '',
                cache: false,
                dataType: 'json',
                beforeSend: function() {
                ...
                success: function(data) {
                    ...
                    $.each(data.data,function(i, v){
                        ...
                        str = '<tr class="'+highlight+'">'+
                                ...
                                '<td align="center">'+((v.status !== 'Processing')?'<i class="fa fa-check-circle-o"></i>':'<i class="blinker">Belum</i>')+'</td>'+
                                '<td>'+((v.berhak_ujian !== null)?v.berhak_ujian:'-')+'</td>'+
                                '<td align="right" class=aksi>'+
                                    '<button type="button" class="btn btn-default btn-xs" title="Klik untuk verifikasi lamaran" onclick="verifikasi_lamaran(\''+v.id_lamaran+'\','+data.page+');"><i class="fa fa-pencil-square-o"></i></button>'+
                                '</td>'+
                            '</tr>';
        ...
        function verifikasi_lamaran(id, page) {
            $('#oldpict').html('');
            $('#form_add').modal('show');
            $('#form_add .title h4').html('Verifikasi Data lamaran');
            $.ajax({
                type: 'GET',
                url: '<?= base_url('api/areafiftyone/lamarans') ?>/page/1/id/'+id,
                dataType: 'json',
                success: function(data) {
                    $('#id').val(data.data[0].id_lamaran);
                    $('#nama').html(data.data[0].nama_lengkap);
                    $('#sekolah').html(data.data[0].asal_sekolah);
                    $('#status').val(data.data[0].status);
                    $('#berhak_ujian').val(data.data[0].berhak_ujian);
                }
            });
        }
        ...
        function save_lamaran() {
            $.ajax({
                type: 'POST',
                dataType: 'json',
                url: '<?= base_url('api/areafiftyone/lamaran') ?>',
        ...
        function delete_lamaran(id, page) {
            bootbox.dialog({
                ...
                buttons: {
                  ...
                  ya: {
                    ...
                    callback: function() {
                        $.ajax({
                            type: 'DELETE',
                            url: '<?= base_url('api/areafiftyone/lamaran') ?>/id/'+id,
                            dataType: 'json',
                            success: function(data) {
                        ...
                        <table class="table table-bordered table-stripped table-hover" id="table_data" width="100%">
                            <thead>
                                <tr>
                                    ...
                                    <th width="17%" class="left">Perusahaan</th>
                                    <th width="25%" class="left">Lowongan</th>
                                    <th width="20%" class="left">Nama Pelamar</th>
                                    <th width="20%" class="left">Nama Sekolah</th>
                                    <th width="5%">Verifikasi</th>
                                    <th width="5%" class="left">Lulus&nbsp;Adm</th>
                                    <th width="5%"></th>
                                </tr>
                            </thead>
                ...
                <div class="col-md-12">
                    <div class="widget-body">
                        <div class="form-group">
                            <label class="col-lg-3 control-label">Nama lamaran:</label>
                        ...
                        <div class="form-group">
                            <label class="col-lg-3 control-label">Lulus Administrasi:</label>
                            <div class="col-lg-8" id="nama">
                                <select name="berhak_ujian" id="berhak_ujian" class="form-control">
