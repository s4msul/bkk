    ...
    <script type="text/javascript">
        $(function() {
            ...
            $('#bt_tambah').click(function() {
                $('#form_add').modal('show');
                reset_form();
                $('#form_add .title h4').html('Form Tambah alumni');
            });
        ...
        function get_list_alumni(p) {
            $('#form-pencarian').modal('hide');
            var id = '';
            $.ajax({
                type : 'GET',
                url: '<?= base_url("api/areafiftyone/alumnis") ?>/page/'+p+'/id/'+id,
                data: '',
                cache: false,
                dataType: 'json',
                beforeSend: function() {
                ...
                success: function(data) {
                    ...
                    $.each(data.data,function(i, v){
                        ...
                        str = '<tr class="'+highlight+'">'+
                                ...
                                '<td>'+v.nama+'</td>'+
                                '<td align="center">'+v.tinggi+'</td>'+
                                '<td align="center">'+v.berat+'</td>'+
                                '<td>'+v.ortu+'</td>'+
                                '<td>'+v.jurusan+'</td>'+
                                '<td align="center">'+v.tahun+'</td>'+
                                '<td>'+v.telp+'</td>'+
                                '<td>'+v.cita+'</td>'+
                                '<td>'+v.status+'</td>'+
                                '<td align="right" class=aksi>'+
                                    '<button type="button" class="btn btn-default btn-xs" onclick="delete_alumni(\''+v.id+'\','+data.page+');"><i class="fa fa-trash-o"></i></button>'+
                                '</td>'+
                            '</tr>';
        ...
        function edit_alumni(id) {
            $('#oldpict').html('');
            $('#form_add').modal('show');
            $('#form_add .title h4').html('Verifikasi Data alumni');
            $.ajax({
                type: 'GET',
                url: '<?= base_url('api/areafiftyone/alumnis') ?>/page/1/id/'+id,
                dataType: 'json',
                success: function(data) {
        ...
        function save_alumni() {
            $.ajax({
                type: 'POST',
                dataType: 'json',
                url: '<?= base_url('api/areafiftyone/alumni') ?>',
        ...
        function delete_alumni(id, page) {
            bootbox.dialog({
                ...
                buttons: {
                  ...
                  ya: {
                    ...
                    callback: function() {
                        $.ajax({
                            type: 'DELETE',
                            url: '<?= base_url('api/areafiftyone/alumni') ?>/id/'+id,
                            dataType: 'json',
                            success: function(data) {
                    ...
                    <div class="col-lg-12">
                        <table class="table table-bordered table-stripped table-hover" id="table_data" width="100%">
                            <thead>
                                <tr>
                                    <th width="3%">No.</th>
                                    <th width="15%" class="left">Nama alumni</th>
                                    <th width="5%">TB</th>
                                    <th width="5%">BB</th>
                                    <th width="20%" class="left">PEKERJAAN ORTU</th>
                                    <th width="15%" class="left">Jurusan</th>
                                    <th width="10%" class="left">Tahun Lulus</th>
                                    <th width="5%" class="left">Telp</th>
                                    <th width="5%" class="left">Cita-cita</th>
                ...
                <div class="col-md-12">
                    <div class="widget-body">
                        <div class="form-group">
                            <label class="col-lg-3 control-label">Nama alumni:</label>
