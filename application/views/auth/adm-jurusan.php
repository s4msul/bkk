    ...
    <script type="text/javascript">
        $(function() {
            ...
            $('#bt_tambah').click(function() {
                $('#form_add').modal('show');
                reset_form();
                $('#form_add .title h4').html('Form Tambah jurusan');
            });
        ...
        function get_list_jurusan(p) {
            $('#form-pencarian').modal('hide');
            var id = '';
            $.ajax({
                type : 'GET',
                url: '<?= base_url("api/areafiftyone/jurusans") ?>/page/'+p+'/id/'+id,
                data: '',
                cache: false,
                dataType: 'json',
                beforeSend: function() {
                    ...
                    $.each(data.data,function(i, v){
                        ...
                        str = '<tr class="'+highlight+'">'+
                                ...
                                '<td align="center" class=aksi>'+
                                    '<button type="button" class="btn btn-default btn-xs" onclick="edit_jurusan(\''+v.id+'\')"><i class="fa fa-pencil"></i></button> '+
                                    '<button type="button" class="btn btn-default btn-xs" onclick="delete_jurusan(\''+v.id+'\','+data.page+');"><i class="fa fa-trash-o"></i></button>'+
                                '</td>'+
                            '</tr>';
        ...
        function edit_jurusan(id) {
            $('#oldpict').html('');
            $('#form_add').modal('show');
            $('#form_add .title h4').html('Form Edit jurusan');
            $.ajax({
                type: 'GET',
                url: '<?= base_url('api/areafiftyone/jurusans') ?>/page/1/id/'+id,
                dataType: 'json',
                success: function(data) {
                    $('#id').val(data.data[0].id);
                    $('#jurusan').val(data.data[0].nama);
                }
            });
        }
        ...
        function save_jurusan() {
            $.ajax({
                type: 'POST',
                dataType: 'json',
                url: '<?= base_url('api/areafiftyone/jurusan') ?>',
        ...
        function delete_jurusan(id, page) {
            bootbox.dialog({
                ...
                buttons: {
                  ...
                  ya: {
                    ...
                    callback: function() {
                        $.ajax({
                            type: 'DELETE',
                            url: '<?= base_url('api/areafiftyone/jurusan') ?>/id/'+id,
                            dataType: 'json',
                            success: function(data) {
                    ...
                    <div class="col-lg-12">
                        <table class="table table-bordered table-stripped table-hover" id="table_data" width="100%">
                            <thead>
                                <tr>
                                    <th width="3%">No.</th>
                                    <th width="90%" class="left">Nama jurusan</th>
                                    <th width="5%"></th>
                                </tr>
                            </thead>
                ...
                <div class="col-md-12">
                    <div class="widget-body">
                        <div class="form-group">
                            ...
                            <div class="col-lg-8">
                              <input type="text" name="jurusan" class="form-control" value="" id="jurusan"/>
                            </div>
                        </div>
                    </div>
                </div>
